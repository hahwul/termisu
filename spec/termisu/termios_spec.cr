...
describe Termisu::Termios do
  ...
  # mode with ICRNL, IXON and OPOST still on: a pasted CRLF arrived as two LFs (Enter
  # indistinguishable from Ctrl+J) and Ctrl+S froze the application. Only c_lflag/c_cflag,
  # mutated inline in `set_mode`, ever reached the terminal. Assert against the FD, not the
  # tracked mode — `current_mode` was right the whole time.
